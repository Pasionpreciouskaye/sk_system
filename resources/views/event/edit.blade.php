{{-- Edit Modal per event --}}
<div x-show="showEditModal && editEventId === {{ $event->id }}" x-cloak
    ...
    <div class="bg-white rounded-xl shadow-lg z-10 p-6 w-full max-w-4xl">
        <form action="{{ route('event.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="md:col-span-2 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="announcement" id="announcement" rows="6" required
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-pink-500 focus:border-pink-500"
                            placeholder="Enter HTML content...">{{ old('announcement', $event->announcement) }}</textarea>
                    </div>
                </div>
                <div class="space-y-4">
                    <label for="file_name" class="block text-sm font-medium text-gray-700">Upload Image</label>
                    <div x-data="{ fileName: '{{ $event->file_name }}' }"
                    ...
                    <p class="text-xs text-gray-500 text-center">Current file: {{ $event->file_name }}</p>
                </div>
            </div>
            <div class="flex justify-start mt-6">
                ...
                <button type="submit"
                    class="px-4 py-2 text-white bg-pink-600 rounded-lg hover:bg-pink-700 transition">
                    Update
                </button>
            </div>
        </form>
