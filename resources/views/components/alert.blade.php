...
<div x-data="{ open: true }" x-init="setTimeout(() => open = false, 3000)" x-show="open"
    ...
    class="fixed top-6 right-6 bg-green-100 border border-green-400 text-green-700 px-6 py-4 rounded-lg shadow-lg z-100"
    role="alert">
...
<div x-data="{ open: true }" x-init="setTimeout(() => open = false, 3000 + ({{ $index }} * 200))" x-show="open"
    ...
    x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-300"
    x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2"
    class="fixed right-6 bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-lg z-100 w-80"
...
<div x-data="{ open: true }" x-init="setTimeout(() => open = false, 3000)" x-show="open"
    ...
    x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-300"
    x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2"
    class="fixed top-6 right-6 bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-lg z-100"
    role="alert">
    <div class="flex items-center justify-between space-x-4">
